...
import type { Semester, Course, StudentProfile } from '../types.ts';
...
const SemesterCard: React.FC<SemesterCardProps> = ({ semester, profile, onUpdateSemester, onRemoveSemester }) => {
    ...
    }, [courses, semester, onUpdateSemester]);
    ...
    const handleDownloadKHS = () => {
        ...
        // Title
        doc.setFontSize(16);
        doc.text(`LAPORAN HASIL STUDI (KHS)`, doc.internal.pageSize.getWidth() / 2, 20, { align: 'center' });
        doc.setFontSize(12);
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="space-y-2">
                ...
                {courses.map(course => (
                    <div key={course.id} className="grid grid-cols-12 gap-2 items-center p-2 bg-gray-50 rounded-md">
                        <div className="col-span-5">
                            <input type="text" value={course.name} onChange={(e) => handleUpdateCourse(course.id, 'name', e.target.value)} className="w-full bg-white text-gray-800 px-2 py-1 rounded-md border border-gray-300"/>
                        </div>
                        <div className="col-span-2">
                            <input type="number" value={course.credits} onChange={(e) => handleUpdateCourse(course.id, 'credits', e.target.value)} className="w-full text-center bg-white text-gray-800 px-2 py-1 rounded-md border border-gray-300"/>
                        </div>
                        <div className="col-span-2">
                           <input type="number" value={course.score ?? ''} onChange={(e) => handleUpdateCourse(course.id, 'score', e.target.value)} className="w-full text-center bg-white text-gray-800 px-2 py-1 rounded-md border border-gray-300" min="0" max="100" step="0.01"/>
                        </div>
                         <div className={`col-span-1 text-center font-bold text-lg ${getGradeDetailsFromScore(course.score).color}`}>
                           {getGradeDetailsFromScore(course.score).letter}
                        </div>
                ...
                {courses.length === 0 && (
                    <p className="text-center text-gray-500 py-4">Belum ada mata kuliah. Tambahkan di bawah.</p>
                )}
            </div>
